export const lastVerified = "2026-06-05";

export const toolSchemas = [
  {
    name: "memory.write",
    title: "Write Memory",
    status: "implemented",
    description: "Store a Mr Mainspring agent memory, canonicalize it, compute hashes, and optionally submit local anchor metadata.",
    input: {
      type: "object",
      required: ["agent_id", "type", "body"],
      properties: {
        agent_id: { type: "string", minLength: 1 },
        type: {
          type: "string",
          enum: ["observation", "decision", "payment", "secret_usage", "system_event"]
        },
        body: { type: "object", additionalProperties: true },
        source: { type: "object", additionalProperties: true },
        anchor: { type: "boolean" },
        memory_id: { type: "string", minLength: 1 },
        prev_anchor_hash: { anyOf: [{ type: "string", minLength: 1 }, { type: "null" }] }
      }
    },
    outputSummary: ["memory_id", "content_hash", "metadata_hash", "anchor_status", "created_at"]
  },
  {
    name: "memory.read",
    title: "Read Memory",
    status: "implemented",
    description: "Read one stored memory by agent and memory id.",
    input: {
      type: "object",
      required: ["agent_id", "memory_id"],
      properties: {
        agent_id: { type: "string", minLength: 1 },
        memory_id: { type: "string", minLength: 1 }
      }
    },
    outputSummary: ["found", "memory"]
  },
  {
    name: "memory.search",
    title: "Search Memory",
    status: "implemented",
    description: "Search stored memories for an agent using the current local JSON-file store.",
    input: {
      type: "object",
      required: ["agent_id"],
      properties: {
        agent_id: { type: "string", minLength: 1 },
        query: { type: "string", default: "" },
        limit: { type: "integer", minimum: 1, maximum: 50 }
      }
    },
    outputSummary: ["agent_id", "query", "count", "results"]
  },
  {
    name: "memory.verify",
    title: "Verify Memory",
    status: "implemented",
    description: "Recompute local memory hashes and report local integrity plus anchor metadata.",
    input: {
      type: "object",
      required: ["agent_id", "memory_id"],
      properties: {
        agent_id: { type: "string", minLength: 1 },
        memory_id: { type: "string", minLength: 1 }
      }
    },
    outputSummary: ["found", "valid", "local_content_hash", "stored_content_hash", "anchor_status", "anchor_id"]
  },
  {
    name: "grimoire.secret.put",
    title: "Store Secret",
    status: "implemented",
    description: "Encrypt and store a scoped secret. The plaintext value is never returned.",
    input: {
      type: "object",
      required: ["agent_id", "name", "type", "value", "scopes"],
      properties: {
        agent_id: { type: "string", minLength: 1 },
        name: { type: "string", minLength: 1 },
        type: {
          type: "string",
          enum: ["casper_private_key_ref", "x402_client_key_ref", "api_key", "webhook_secret"]
        },
        value: { type: "string", minLength: 1 },
        scopes: { type: "array", minItems: 1, items: { type: "string", minLength: 1 } }
      }
    },
    outputSummary: ["status", "secret"]
  },
  {
    name: "grimoire.secret.list",
    title: "List Secrets",
    status: "implemented",
    description: "List secret metadata for an agent without exposing secret values.",
    input: {
      type: "object",
      required: ["agent_id"],
      properties: {
        agent_id: { type: "string", minLength: 1 }
      }
    },
    outputSummary: ["agent_id", "count", "secrets"]
  },
  {
    name: "grimoire.policy.set",
    title: "Set Policy",
    status: "implemented",
    description: "Create or update an allowlisted spending/access policy and deterministic policy hash.",
    input: {
      type: "object",
      required: [
        "agent_id",
        "policy_id",
        "allowed_urls",
        "allowed_methods",
        "allowed_asset",
        "max_amount_per_call",
        "max_amount_per_period",
        "period_seconds"
      ],
      properties: {
        agent_id: { type: "string", minLength: 1 },
        policy_id: { type: "string", minLength: 1 },
        enabled: { type: "boolean" },
        allowed_urls: { type: "array", minItems: 1, items: { type: "string", format: "uri" } },
        allowed_methods: { type: "array", minItems: 1, items: { type: "string", minLength: 1 } },
        allowed_asset: { type: "object", additionalProperties: true },
        max_amount_per_call: { type: "string", pattern: "^\\d+(\\.\\d+)?$" },
        max_amount_per_period: { type: "string", pattern: "^\\d+(\\.\\d+)?$" },
        period_seconds: { type: "integer", minimum: 1 },
        secret_scopes: { type: "array", items: { type: "string", minLength: 1 }, default: [] }
      }
    },
    outputSummary: ["status", "policy"]
  },
  {
    name: "grimoire.policy.get",
    title: "Get Policy",
    status: "implemented",
    description: "Read one policy plus current local spend metadata.",
    input: {
      type: "object",
      required: ["agent_id", "policy_id"],
      properties: {
        agent_id: { type: "string", minLength: 1 },
        policy_id: { type: "string", minLength: 1 }
      }
    },
    outputSummary: ["found", "policy"]
  },
  {
    name: "payment.fetch",
    title: "x402 Payment Fetch",
    status: "pre-settlement",
    description: "Create a durable x402 payment intent after policy approval. With request_challenge=true, make the first HTTP request and persist any 402 requirements without claiming settlement.",
    input: {
      type: "object",
      required: ["agent_id", "policy_id", "method", "url"],
      properties: {
        agent_id: { type: "string", minLength: 1 },
        policy_id: { type: "string", minLength: 1 },
        method: { type: "string", minLength: 1, default: "GET" },
        url: { type: "string", format: "uri" },
        expected_amount: { type: "string", pattern: "^\\d+(\\.\\d+)?$" },
        idempotency_key: { type: "string", minLength: 1 },
        request_challenge: { type: "boolean", default: false }
      }
    },
    outputSummary: [
      "allowed",
      "payment_id",
      "status",
      "next_state",
      "settlement",
      "requirements_json",
      "challenge",
      "settlement_blocker"
    ]
  },
  {
    name: "payment.receipt",
    title: "Read Payment Receipt",
    status: "implemented",
    description: "Return persisted payment intent and receipt metadata. Signed payloads and secrets are never returned.",
    input: {
      type: "object",
      required: ["payment_id"],
      properties: {
        payment_id: { type: "string", minLength: 1 }
      }
    },
    outputSummary: ["found", "payment_id", "intent", "receipt"]
  },
  {
    name: "audit.tail",
    title: "Tail Audit Events",
    status: "implemented",
    description: "Return recent audit events for memory, Grimoire, payment, and anchor activity.",
    input: {
      type: "object",
      properties: {
        agent_id: { type: "string", minLength: 1 },
        event_type: { type: "string", minLength: 1 },
        limit: { type: "integer", minimum: 1, maximum: 200 }
      }
    },
    outputSummary: ["count", "events"]
  }
];
